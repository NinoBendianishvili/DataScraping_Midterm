...
class StateElectionScraper:
    ...
    def scrape_single_state(self, state_name: str, state_url_path: str) -> List[ElectionResult]:
        ...
        for year_data_dict in parsed_yearly_data:
            ...
            if year is None: continue # Must have year

            # Get data from parsed dict (defaults to None if key missing)
            ...
            try:
                # Create or get YearData object (reuse if same leaders/votes across states for a year)
                # For simplicity here, we create a new one each time, but caching is possible
                year_obj = YearData(
                    year=year,
                    dem_leader=dem_leader, # Pass leader (likely None)
                    rep_leader=rep_leader, # Pass leader (likely None)
                    total_votes=total_votes  # Pass total votes (likely None)
                )

                # Create ElectionResult, linking StateData, YearData, and percentages
