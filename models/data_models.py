...
# Defining three separate blueprints for our election data.
# Updated to include total_votes in YearData again.

from typing import Optional, Dict # Import Dict

# --- Model 1: State Information ---
class StateData:
    """
    ...
    def __init__(self,
        ...
        self.electoral_votes: Optional[int] = electoral_votes # Positive check removed
        ...
        self.total_population: Optional[int] = total_population # Positive check removed
...
# --- Model 2: Year-Specific Information ---
class YearData:
    """
    ...
    def __init__(self,
        ...
        # Check and store total votes
        self.total_votes: Optional[int] = self._check_total_votes(total_votes)

    def _check_total_votes(self, votes: Optional[int]) -> Optional[int]:
        """ Check: total votes should be an integer >= 0 (or None)."""
        # Allow None through
        if votes is None:
            return None
        # Check type
        if not isinstance(votes, int):
             try: # Handle floats like 12345.0
                if isinstance(votes, float) and votes.is_integer() and votes >= 0:
                    return int(votes)
                else:
                    print(f"Warning ({self.year}): Expected integer >= 0 for total votes, got {type(votes)} '{votes}'. Storing 'None'.")
                    return None
             except Exception: # Catch any conversion/comparison error
                 print(f"Warning ({self.year}): Could not process total votes value '{votes}'. Storing 'None'.")
                 return None
        # Check value
        if votes < 0:
            print(f"Warning ({self.year}): Total votes '{votes}' is negative. Storing 'None'.")
            return None
        return votes
    ...
    def __repr__(self) -> str:
        ...
        return (f"YearData(year={self.year}, "
                ...
                f"TotalVotes={self.total_votes})") # Added TotalVotes

# --- Model 3: Linking State, Year, and Percentages ---
